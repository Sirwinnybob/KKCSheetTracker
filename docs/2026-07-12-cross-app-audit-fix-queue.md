...
**Watcher-side consolidation** was then run directly against the real job 646 tracker directory by
invoking `ready_jobs_watcher.metadata_cache.consolidate_cnc_tracker` /
`consolidate_hardwoods_tracker` from the project's own `.venv` Python (non-destructive,
`compact=False`, so no event streams were rotated/deleted — the watcher's own per-tracker-dir file
lock, `.consolidate.lock`, makes this safe to run alongside the live watcher process). Result:
`CNC\.tracker\consolidated.json` shows page 1 `complete` at `2026-07-13T14:28:44.364102Z` --
byte-for-byte the same timestamp as the tablet's winning action, an exact match with the
Android-rendered state. `.metadata\hardwoods\.tracker\consolidated.json` correctly merged all 3
real hardwood actions from both tablets (2 `FACE_FRAME_CUT_LIST` rows + 1 `DOOR_CUT_LIST` row).

**False-alarm note for future debugging:** a `DOOR_CUT_LIST` action briefly appeared to be lost
during this investigation -- it was present when read directly off the tablet's local storage via
`adb`, but missing from the PC-side `Y:\Ready Jobs` copy of the same file moments later. This was
Syncthing propagation lag (a few minutes), not a bug: re-reading the `Y:` copy after some time
passed showed it had caught up and converged correctly. Comparing tablet-local (`adb shell cat
/storage/emulated/0/Ready Jobs/...`) and PC-side (`Y:\Ready Jobs\...`) copies of the same tracker
file without accounting for Syncthing's asynchronous propagation will produce spurious
data-loss-looking diffs; always re-check both sides again before concluding data was actually lost.
...
### Post-loop field verification (2026-07-13) — CRITICAL regression found and fixed in the AUD-08 build

Installed the round-2 `assembleRelease` build on a real shop tablet (SM-X808U, PIN 1965) to
field-verify the loop's fixes. Found that AUD-08's commit (`89c42dc`) broke reading of ALL
pre-migration legacy tracker JSON on-device: every job's `CNC\.tracker\consolidated.json` (real,
intact production history, e.g. job 314 had ~80 real "complete" actions from 2026-06-22 through
2026-07-09) was being silently dropped as "malformed," making every job on every tablet show 0%
complete despite the underlying data being fully intact on the shared drive. Confirmed via
`adb logcat` (`KKC_PROGRESS: Skipping malformed tracker file`) across all 11 jobs present on that
tablet, and via `consolidated.json` read directly off `/storage/emulated/0/Ready Jobs`.

**Root cause:** `TrackerAction` (Models.kt) gained non-null `lamport`/`eventId` fields with Kotlin
defaults. Gson has no Kotlin-default-aware adapter registered (`ProgressStore.kt:182`, plain
`GsonBuilder()`), so for legacy JSON missing those keys it leaves `eventId` as a raw JVM `null`
(confirmed empirically) instead of applying the `= ""` default. `ProgressStore.sanitizeAction()`
then reconstructs the object via `TrackerAction`'s real constructor, passing that null into a
Kotlin non-null parameter, which throws `NullPointerException` (Kotlin's compiler-inserted
`Intrinsics.checkNotNullParameter`). The exception is caught by the surrounding
`catch (_: Exception)` and logged as "skipping malformed file" -- silently, with no crash and no
indication of real data loss.

**Fix:** `ProgressStore.kt` `sanitizeAction()` -- cast `action.eventId` through `(action.eventId as
String?).orEmpty()`, matching the defensive-cast pattern already used for `file`/`action`/
`timestamp` in the same function. Verified empirically via a throwaway Gson probe test (removed
after use) that `HardwoodTrackerAction` does NOT hit the same failure (Gson happens to produce
`""` there, not null, because every constructor parameter has a default vs. CNC's mix of
required/defaulted params) -- so the equivalent one-line change in `HardwoodsProgressStore.kt`
`sanitizeAction()` is defense-in-depth/consistency hardening against Gson-version-dependent
behavior, not a fix for a currently-active bug. Both are one-line, minimal, root-cause fixes with
no behavior change for already-working inputs.

**Regression tests added (TDD, red before fix):**
- `ProgressStoreTest.loadAllProgressToleratesPreAud08LegacyJsonMissingLamportAndEventId` -- writes
  genuine pre-migration JSON text (no `lamport`/`eventId` keys) and asserts it loads.
- `HardwoodsProgressStoreTest.loadAllProgressToleratesPreAud08LegacyJsonMissingLamportAndEventId`
  -- same shape, pins the currently-safe Hardwoods behavior against future Gson changes.

**Verification:** `:app:testDebugUnitTest :updater-agent:testDebugUnitTest --rerun-tasks` and
`assembleRelease` both green. Reinstalled the fixed build on the same real tablet and confirmed
against live production data: dashboard recovered from "0 of 466 sheets, 0%" to "399 of 453
sheets, 88% complete, 27 jobs tracked," matching real per-job history (e.g. job 332 11/24, job 582
2/3). Job 314's "002 REMAKE" sheet -- the one item deliberately toggled complete/incomplete during
this same field session with user authorization -- correctly reads back 0/1, confirming that test
left no lasting state change.

**Severity:** this would have zeroed out visible progress on every job, on every tablet, the
moment tomorrow's release build was installed fleet-wide, while leaving the real underlying data
completely intact and silently unreadable. The original loop's unit tests did not catch this
because their legacy-JSON fixtures were always generated by serializing a properly-constructed
`TrackerAction`/`HardwoodTrackerAction` (via `gson.toJson`), which always includes the new fields
-- never exercising genuine pre-migration JSON text missing them entirely, which is the real shape
every existing production file has.

**Files changed:** `app/src/main/java/com/kkc/sheettracker/data/ProgressStore.kt`,
`app/src/main/java/com/kkc/sheettracker/data/HardwoodsProgressStore.kt`,
`app/src/test/java/com/kkc/sheettracker/data/ProgressStoreTest.kt`,
`app/src/test/java/com/kkc/sheettracker/data/HardwoodsProgressStoreTest.kt`.

**Learning for future iterations:** when a Kotlin data class gains new non-null fields with
defaults and is deserialized via plain Gson (no Kotlin adapter), any legacy JSON fixture used in
tests must be raw JSON text missing those keys, not a round-tripped `gson.toJson(properlyBuiltObj)`
-- the latter always passes and hides exactly this class of regression. Any future field added to
`TrackerAction`/`HardwoodTrackerAction` should get the same defensive-cast treatment as the other
fields in `sanitizeAction()`, or the model should switch to a Kotlin-aware Gson adapter.
