...
- `Done evidence`: Hoisted `syncCncToHardwoods` listener and its `HardwoodsRepository` instantiation from `MultiBackStackNavigation` up into `AppNavigation` so it is active for both legacy and multi-stack paths. Ran tests `.\gradlew.bat :app:testDebugUnitTest --tests com.kkc.sheettracker.navigation.*`.
...
## 2026-07-07 - Medium #23 sync listener only in multi-stack
...
- Test commands: `.\gradlew.bat :app:testDebugUnitTest --tests com.kkc.sheettracker.navigation.*`.
- Result: passed.
- Verification note: Hoisted `syncCncToHardwoods` and `HardwoodsRepository` to `AppNavigation` top level.
