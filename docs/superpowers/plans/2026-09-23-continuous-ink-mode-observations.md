...
- `KIND`: `BUG` (wrong behavior), `RACE` (ordering/concurrency), `PERF` (compute, allocation, IO), `RISK` (fragile, could break later), `CLEANUP` (duplication, dead code, naming), `TEST-GAP` (untested behavior).
