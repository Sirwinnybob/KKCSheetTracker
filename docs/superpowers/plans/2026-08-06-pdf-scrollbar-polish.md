...
- [ ] During the carousel's exit fade specifically (not just its motion): release a drag several times, including fast releases far from the drag's start page, and watch for a content flicker/jump in the departing carousel — `focusIndex` falls back from `dragIndex` to `currentEntryIndex` on release, and if that fallback lags a frame behind the parent's `currentPage` state, the now-visible 200ms fade-out could transiently show the wrong page before catching up. If observed, fix by capturing the last dragged index into a `remember`ed `lastKnownFocusIndex` (same pattern as `lastKnownFillHeightPx`) rather than trusting `currentEntryIndex`'s recomposition timing.
