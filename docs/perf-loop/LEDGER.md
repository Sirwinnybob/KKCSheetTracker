...
| U2 | Viewers + markup | `ui/viewer/SheetViewerScreen.kt`, `UnifiedReferenceViewer.kt`, `ReferencePdfViewerScreen.kt`, `ui/components/ReferencePdfPane.kt`, `ui/markup/` | in-progress |
...
|------|-----------|--------|--------------|--------|
| 2026-06-27 | U1 Dashboards | _(this commit)_ | Recomposition cleanup matching the existing CNC pattern. `CncDashboardContent` already wrapped its widget build in `remember(dashboard)`, but Hardwoods/Assembly/Specialty dashboards built their `DashboardWidgetModel` lists inline on every recomposition. Wrapped all three `build*DashboardWidgets` calls in `remember(...)` keyed on their inputs, and wrapped the inline `totalCounts` folds in `remember(summaries)` (in both `UnifiedModeDashboardScreen` Hardwoods content and `HardwoodsDashboardScreen`). build* factories verified pure. `AssemblyDashboardScreen`/`SupplyDashboardScreen` already well-remembered (state-backed lists, keyed LazyColumns, IO-backed loads); HardwoodsDashboardScreen heavy derivation already in produceState+Dispatchers.IO. No behavior change — same widgets, fewer per-recomposition allocations. | `assembleDebug` + `testDebugUnitTest` BUILD SUCCESSFUL (15s) |
