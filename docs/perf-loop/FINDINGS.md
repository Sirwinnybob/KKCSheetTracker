...
### ✅ DONE (2026-06-28): N+1: per-job `updateJobInState` re-scans every job via `listJobsFromCacheOnly()`
Implemented: added `getMergedJobInfo(folderName)` to engine; repointed all 4 callers. See git log.
...
### ⏸️ PARKED (2026-06-28, owner declined): Deep-parse path calls full `listJobs()` to get one job's lineupPosition (compounds the deep-load N+1)
Left unimplemented on purpose: a cheap memoization signature (job_board.json + dir listing) can't detect per-job deployment_gate.json changes mid-batch, risking stale job ordering/visibility in the hot path. Higher risk than the rest of the backlog; owner chose to skip for now. Revisit only with a deliberate signature design that includes gate-file state.
...
### ✅ DONE (2026-06-28): `getCncSnapshot` rebuilds the CNC search index on every call; some callers discard it
Fixed via in-memory memoization keyed by static signature (cncSearchByJob), invalidated with staticByJob. See git log.
...
### ✅ DONE (2026-06-28): Per-job `updateJobInState` rebuilds the entire CNC search index each call
Fixed: added updateJobsInState(folders) — one search-index filter + one emission for the whole deep-load batch; updateJobInState delegates to it. Owner approved batch-appearance trade-off. See git log.
...
### ✅ DONE (2026-06-28): Dead warmup branch in `TrackerChangeMonitor.flushPendingNow` (possible latent bug)
Resolved by deletion (warmup deferral already in scheduleFlushLocked; block was unreachable). See git log.
...
### ✅ DONE (2026-06-28, contained MVP): CNC `ProgressStore` write path: full JSON re-parse + re-serialize per appended action
Batched the multi-append loops (markSheetComplete/resolveBadPartsOnSheet/resolveSpecificBadParts) into one load→append-all→save→bump via new `appendActions`. Full in-memory cacheByJob + async persist (matching Hardwoods) NOT done — larger change, still open if more is wanted. See git log.
...
### ✅ DONE (2026-06-28): Update check runs APK-archive parsing + external-storage scan on the main thread
Fixed: checkForUpdates now scans on a background thread (scanForUpdates) and applies Compose state + dialog on main (applyUpdateScan via runOnUiThread). See git log.
...
### ✅ DONE (2026-06-28): Minor: `findUpdateDirectory`/`findReleaseUpdateDirectory` duplicate the custom-path read + job-folder walk
Fixed: extracted shared resolveUpdateDir(subfolders, recordResolved) helper. See git log.
...
### ✅ DONE (2026-06-28): JobBrowserScreen derives all-job status counts synchronously in `remember` (legacy path)
Fixed: jobUiStates now produceState; appState counts instant, fallback counts resolved on IO. Cards render at final size (zero-count placeholder), bar/numbers animate in. Owner confirmed flicker trade-off (cards must keep final size — satisfied). See git log.
...
### ✅ DONE (2026-06-28): `TrackerChangeMonitor` flushJob self-clear never fires (`=== this` compares Job to CoroutineScope)
Fixed: compare `flushJob === coroutineContext[Job]`. See git log.
