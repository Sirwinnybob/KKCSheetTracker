...
| `C:\Scripts\Hours Tracker\backend\molding_hidden.json`, `%DATA_DIR%\molding_hidden.json` | Hours Tracker | Crown-only Hidden tag, keyed by `moldingId`; HT-local sidecar store that flows into published `moldings_cache\library.json`'s `hidden` field to exclude hidden crowns on tablets |
