...
| Hours Tracker | Manages digital hours/admin metadata and some global Ready Jobs admin files; also ships a full RJW-replacement worker (see caveat below) | `C:\Scripts\Hours Tracker` |
...
> **RJW-replacement worker (as of 2026-08-18):** `backend/ready_jobs_worker_core` (Hours Tracker repo) is a from-scratch port capable of owning EVERY row in the Ownership Map below currently labeled "Ready Jobs Watcher" — deployment gates, cache_static/cache_index, CNC/hardwoods consolidation, cabinet_sheet_index, moldings sync, dark-mode PDFs, 3D GLB conversion, sync-conflict resolution, duplicate-folder guard, job rename/reparse, deployment-gate timers, hidden-gate bootstrap, and construction-method (Face-Frame/Frameless) auto-detection. Verified via a shadow-write soak test against a full real-tree copy (byte-exact `cache_static.json`/`cache_index.json` match on 15/16 live jobs, clean cold boot on 3455 real files) before any production cutover. Runs as its own container/process (`python -m ready_jobs_worker_core`, NOT the `main_v2.py` web app — that process refuses to run a real writer in-process by design), sharing the web GUI's `/data` state dir and the real `/jobs` mount. **This does NOT mean Ready Jobs Watcher is retired everywhere** — whether the real Windows RJW process or this worker is the live writer for a given job tree is a per-site deployment fact, not a fixed rule: check `docker ps` for a running `hourtracker-worker`-style container AND whether `C:\Scripts\Ready Jobs Watcher`'s python process is still running before trusting any "Owner" cell below at face value on a site that may have cut over. The worker's own operational log is new local state — see `worker.log` in Local State below.
...
Hours Tracker's RJW-replacement worker (only if `hourtracker-worker` is actually running — check `docker ps` first):
...
| Assuming every "Ready Jobs Watcher" owner cell in this map is still the live writer | As of 2026-08-18, Hours Tracker ships a verified drop-in replacement worker (`ready_jobs_worker_core`) covering every RJW-owned row. Which one actually owns writes on a given site is a deployment fact, not fixed by this document -- check `docker ps` for a running worker container and whether the real RJW Windows process is still up before trusting an "Owner" cell at face value |
