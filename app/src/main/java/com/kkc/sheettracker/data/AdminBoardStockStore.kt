...
fun loadAdminBoardStock(baseDir: File, jobFolderName: String): List<AdminBoardStockItem> {
    ...
    return runCatching {
        ...
        entries.mapNotNull { element ->
            ...
            val ripLength = obj.get("ripLength")
                ?.takeIf { !it.isJsonNull && it.isJsonPrimitive && it.asJsonPrimitive.isNumber }
            ...
            AdminBoardStockItem(
                ...
                mode      = mode,
                ripLength = ripLength,
                createdAt = obj.get("createdAt")?.takeIf { !it.isJsonNull }?.asString.orEmpty(),
                createdBy = obj.get("createdBy")?.takeIf { !it.isJsonNull }?.asString.orEmpty(),
                moldingId = moldingId,
                type      = type
            )
        }
