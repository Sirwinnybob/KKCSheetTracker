...
fun MixActionDialog(
    ...
    pendingDeletionFilename?.let { deletion ->
        AlertDialog(
    ...
    pendingReplacement?.let { replacement ->
        ...
        AlertDialog(
            ...
            text = {
                Text("The current ${replacement.name} version from $archivedAt will be archived before the replacement is compiled.")
            },
            confirmButton = {
                TextButton(onClick = {
                ...
                if (content.generationBlockedByExternal) {
                    ...
                    content.externalMixes.forEach { entry ->
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                            ...
                            TextButton(onClick = { pendingDeletionFilename = entry.mixFilename }) {
                    ...
                    content.activeMixes.forEach { entry ->
                        Column(modifier = Modifier.fillMaxWidth()) {
                            ...
                            Text("Compile: ${compileStatus(entry)}", style = MaterialTheme.typography.bodySmall)
                            TextButton(onClick = { pendingReplacement = entry }) {
