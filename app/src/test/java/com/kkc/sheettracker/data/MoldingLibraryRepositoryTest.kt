...
class MoldingLibraryRepositoryTest {
    ...
    @Test
    fun fetchLibrary_parsesFrameStyleWhenPresent() {
        val baseDir = Files.createTempDirectory("molding-repo-test-frame-style").toFile()
        val cacheDir = File(baseDir, ".metadata/moldings_cache").apply { mkdirs() }
        val json = """
            {
              "categories": ["Crown"],
              "moldings": [
                {"id": "Crown:105", "category": "Crown", "fileId": "105", "name": "3 1/4\" Flat", "frameStyle": "face_frame"},
                {"id": "Crown:106", "category": "Crown", "fileId": "106", "name": "Cove", "frameStyle": null}
              ]
            }
        """.trimIndent()
        File(cacheDir, "library.json").writeText(json)
        ...
        assertEquals("face_frame", library.moldings[0].frameStyle)
        assertEquals(null, library.moldings[1].frameStyle)
    }
